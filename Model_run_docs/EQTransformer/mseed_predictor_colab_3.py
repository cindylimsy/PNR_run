...
edited for Google Colab use by Cindy Lim Shin Yee (University of Bristol) on 5th Aug 2021
...
def _mseed2nparry(args, matching, time_slots, comp_types, st_name):
    ...
    st.filter(type='highpass', freq = 50, zerophase=True)
    ...
    tim_shift = (3-(args['overlap']*3))
    next_slice = start_time+3  
...
def _output_writter_prediction(meta, predict_writer, csvPr, matches, snr, detection_memory, idx):
    ...
    for match, match_value in matches.items():
        ev_strt = start_time + timedelta(seconds= match/2000)
        ev_end = start_time + timedelta(seconds= match_value[0]/2000)
        ...
        if len(doublet) == 0: 
            ...
            if match_value[3]: 
                p_time = start_time+timedelta(seconds= match_value[3]/2000)
            else:
                p_time = None
            ...
            if match_value[6]:
                s_time = start_time+timedelta(seconds= match_value[6]/2000)
            else:
                s_time = None
