...
class DataReader():
    ...
    def __init__(self, format="numpy", config=DataConfig(), **kwargs):
        ...
        if format in ["numpy", "mseed"]:
            ...
            self.s3fs = s3fs.S3FileSystem(anon=kwargs["anon"], key=kwargs["key"], secret=kwargs["secret"], 
    ...
    def generate_label(self, data, phase_list, mask=None):
        ...
        if self.label_shape == "gaussian":
            label_window = np.exp(-(np.arange(-self.label_width//2,self.label_width//2+1))**2/(2*(self.label_width/5)**2))
        elif self.label_shape == "triangle":
            label_window = 1 - np.abs(2/self.label_width * (np.arange(-self.label_width//2,self.label_width//2+1)))
        else:
...
class DataReader_test(DataReader):
    ...
    def dataset(self, batch_size, num_parallel_calls=2, shuffle=False, drop_remainder=False):
        dataset = dataset_map(self, output_types=(self.dtype, self.dtype, "string", "int64", "int64"),
...
def test_DataReader():
    ...
    data_reader = DataReader_train(data_list = "test_data/selected_phases.csv",
                                   data_dir = "test_data/data/")

    read(data_reader)

    data_reader = DataReader_train(format="hdf5",
                                   hdf5 = "test_data/data.h5",
                                   group = "data")

    read(data_reader)


    data_reader = DataReader_test(data_list = "test_data/selected_phases.csv",
                                  data_dir = "test_data/data/")

    read(data_reader)

    data_reader = DataReader_test(format="hdf5",
                                  hdf5 = "test_data/data.h5",
                                  group = "data")

    read(data_reader)

    data_reader = DataReader_pred(format="numpy",
                                  data_list = "test_data/selected_phases.csv",
                                  data_dir = "test_data/data/")

    read(data_reader)

    data_reader = DataReader_pred(format="mseed",
                                  data_list = "test_data/mseed_station.csv",
    ...
    data_reader = DataReader_mseed_array(data_list = "test_data/mseed.csv",
                                         data_dir = "test_data/waveforms/",
